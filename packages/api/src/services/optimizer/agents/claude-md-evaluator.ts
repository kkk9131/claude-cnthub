 ...
 * CLAUDE.md 評価結果
 */
export interface ClaudeMdEvaluation extends EvaluationResult {
  /** 最適化結果への参照 */
  optimizeResult: ClaudeMdOptimizeResult;
}

const TARGET_LINE_COUNT = 100;

/**
 * CLAUDE.md の最適化結果を評価
 *
 * @param optimizeResult - 最適化結果
 * @param projectPath - プロジェクトのルートパス
 * @returns 評価結果
 */
export async function evaluateClaudeMd(
  optimizeResult: ClaudeMdOptimizeResult,
  projectPath: string
): Promise<ClaudeMdEvaluation> {
  const issues: string[] = [];
  ...
  if (lineCount > TARGET_LINE_COUNT) {
    issues.push(
      `行数が${lineCount}行で、目標の${TARGET_LINE_COUNT}行を超えています`
    );
    suggestions.push(
      `さらに${lineCount - TARGET_LINE_COUNT}行削減する必要があります`
    );
    suggestions.push("詳細な説明をrules/やreferences/に分離してください");
  }

...
| R-10 | Plugin Viewer UI (統合方式) | 中 |
| R-11 | セッション一覧 (プロジェクト切替) | 中 |
| R-12 | ノードエディタ (get/export 操作) | 中 |
...
## [追記 2026-01-02] UI 要件 (Viewer 統合)

### R-10: Plugin Viewer UI

Plugin 起動時に `http://localhost:3048/viewer` でアクセス可能な UI。
...
─────────────────────────────────────────
packages/
├── api/          # Backend (Port 3048)
├── web/          # Frontend (Port 5173) ← 廃止予定
└── shared/

After (統合後)
─────────────────────────────────────────
packages/
├── api/          # Backend + 静的配信 (Port 3048)
└── shared/
...
**アクセス**: `http://localhost:3048/viewer`
**配信**: Backend API から静的配信
...
```
┌───────────────────────────────────────────────────────────────────────────────┐
│  🔌 cnthub Viewer (localhost:3048/viewer)                                      │
├─────────────────┬─────────────────────────────────────────────────────────────┤
│                 │                                                             │
│  Project ▼      │              ノード操作画面                                  │
│  ┌───────────┐  │                                                             │
...
左サイドバー: セッション一覧 (IDE ファイルツリー風)
メイン画面: ノードエディタ (現在コンテキスト + 接続)
```
...
| 操作 | UI アクション | 処理 | 対応コマンド |
|------|-------------|------|-------------|
| **get** | リスト → 中心にドラッグ、線を接続 | 選択セッションのコンテキストを注入 | `/cnthub:get` |
| **export** | 中心から線を削除 | コンテキストを要約して新 Session 保存 | `/cnthub:export` |
...
- ドラッグで線を接続 = get (コンテキスト注入)
- 線をクリック削除 = export (要約保存)
```
...
bun run dev:api
    │
    └── GET /viewer → plugin/ui/index.html を配信
```
...
R-10 (Viewer UI)
    │
    ├── R-08 (/cnthub:get) ← get 操作
    ├── R-09 (/cnthub:export) ← export 操作
    └── R-02 (PostToolUse) ← observations データ
```
